...
# =========
KG_NAME = 'Power'
#assert KG_NAME in ['MetaQA', 'FB15k-237','Power', 'WebquestionsSP-tiny']
# =========
HOPS = 1
...
# =========
if KG_NAME == 'Power':
    QA_PAIRS_PATH = f'../QA/Power/qa_%s_{str(HOPS)}hop.txt'
#else:
...
KG_EMBED_PATH = f'../kg_embeddings/{KG_NAME}/' \
...
model = Answer_filtering_module(entity_embeddings=entity_embeddings, embedding_dim=embedding_dim, vocab_size=vocab_size,
                                ...
                                relation_dim=relation_dim,head_bn_filepath=head_bn_path, score_bn_filepath=score_bn_path)
if load_from:
...
TRAINING_RESULTS_DIR = os.path.join('.', '_'.join([KG_NAME, str(HOPS), 'hop',
...
for epoch_idx in range(N_EPOCHS):
    ...
    for batch_questions_index, batch_questions_length, batch_head_entity, batch_answers, max_sent_len in epoch_process:
        model.zero_grad()
        #print(batch_questions_length.cuda().to("cpu"))
