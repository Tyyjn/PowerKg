...
class Answer_filtering_module(torch.nn.Module):
    def __init__(self, entity_embeddings, embedding_dim, vocab_size, word_dim, hidden_dim, fc_hidden_dim, relation_dim,
        ...
        self.fc_lstm2hidden = torch.nn.Linear(hidden_dim * 3, fc_hidden_dim, bias=True)
        ...
        self.fc_hidden2relation = torch.nn.Linear(fc_hidden_dim, self.relation_dim, bias=True)
        ...
        #print(hidden_dim)
        self.BiLSTM = torch.nn.LSTM(embedding_dim, hidden_dim, 2,batch_first=True,bidirectional=True)
        ...
        #print(head_bn_params_dict)
        for key in head_bn_params_dict.tolist():
            #print(head_bn_params_dict.tolist()[key])
            self.head_bn.data = torch.tensor(head_bn_params_dict.tolist()[key])
            #print(self.head_bn.data)
        self.score_bn = torch.nn.BatchNorm1d(2)
        score_bn_params_dict = np.load(score_bn_filepath,allow_pickle=True)
        for key in score_bn_params_dict.tolist():
            self.score_bn.data = torch.tensor(score_bn_params_dict.tolist()[key])
    ...
    def forward(self, question, questions_length, head_entity, tail_entity, max_sent_len):
        ...
        #print(question.unsqueeze(0))
        #print(embedded_question.to("cpu"))
        print(questions_length.to("cpu"))
        packed_input = pack_padded_sequence(embedded_question.to("cpu"),questions_length.to("cpu"), batch_first=True)
        print(packed_input)
    ...
    def get_ranked_top_k(self, question, questions_length, head_entity, max_sent_len, K=5):
        ...
        packed_output, _ = self.BiLSTM(packed_input)
        output, _ = pad_packed_sequence(packed_output, batch_first=True, padding_value=0.0, total_length=max_sent_len)
        output = self.attention_layer(output.permute(1, 0, 2), questions_length.cpu())
        output = torch.nn.functional.relu(self.fc_lstm2hidden(output))
        relation_embedding = self.fc_hidden2relation(output)
        pred_answers_score = self.complex_scorer(self.entity_embedding_layer(head_entity), relation_embedding)
